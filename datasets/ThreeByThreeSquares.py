...
freequent_patterns_freqquency = 2. / 12.
less_freequent_patterns_freqquency = 1. / 12.


class ThreeByThreeSquares(Dataset):
    """
    This class manage the dataset ThreeByThreeSquares, properties of the datasets are uniquely determined
    ...
    def _dataset_id(self, params):
        ...
        ThreeByThreeSquares.check_params_impl(params)
    ...
    def load_data(self):
        ...
        threeByThreeSq = self.generate_dataset(DATASET_SIZE)

        len_of_dataset = len(threeByThreeSq)

        perm = np.random.permutation(len_of_dataset)

        data = threeByThreeSq[perm].astype(np.float32)
    ...
    @property
    def likelihood(self):
        patterns_and_likelihoods = [((np.asarray(i) + 1) / 2, freequent_patterns_freqquency) for i in
                                    freequent_patterns]
        patterns_and_likelihoods += [((np.asarray(i) + 1) / 2, less_freequent_patterns_freqquency) for i in
                                     less_freequent_patterns]
