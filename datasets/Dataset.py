...
def check_dataset_keys_not_loop(dataset_keys):
...
#TODO why add_default_parameters and process_params seems to do the same thing?
    ...
    def dataAugmentation_id(self, params):

        id = ''
        for pert in params.get("dataAugmentation", []):
            if id == '':
                id = Augmentations.get_id(pert)
            else:
                id += "_" + Augmentations.get_id(pert)

        return id


    def dataPerturbation_id(self, params):

        id = ''
        for pert in params.get("dataPerturbation", []):
            if id == '':
                id = Perturbations.get_id(pert)
            else:
                id += "_" + Perturbations.get_id(pert)
...
#TODO see how this is done in Models and Networks
    ...
    def dataset_id(self, params):
        ...
        id_dataAugmentations = self.dataAugmentation_id(params)
        if id_dataAugmentations is not '':
            id_dataAugmentations = '-dA_' + id_dataAugmentations

        id_dataPerturbations = self.dataPerturbation_id(params)
        if id_dataPerturbations is not '':
            id_dataPerturbations = '-dP_' + id_dataPerturbations

        return self._dataset_id(params) + id_dataAugmentations + id_dataPerturbations
    ...
    def dataset_map(self, dataset, datasets_tuple):
        ...
        def get_function(n):
            # if you want to replace the data with something different, for instance
            # by sampling a discrete image from a continuous one, you should act here
            return full_data[n]
        ...
        def apply_py_function(n):
            ...
            #else:
            #    pdb.set_trace()
            return tuple(tf.numpy_function(get_function, [n], output_types))
    ...
    def get_dataset_iterator(self, batch_size, dataset_str, shuffle, repeat, augment, perturb):
        ...
        if dataset_str == TRAIN:
            ...
            if hasattr(self,"perturbed_train_set_x") and self.perturbed_train_set_x is not None:
                ...
                is_perturbed = True
                raise Exception("'perturbed_train_set_x' is now obsolte, talk to ****")
            '''
            ...
            if hasattr(self,"perturbed_validation_set_x") and self.perturbed_validation_set_x is not None:
                ...
                is_perturbed = True
                raise Exception("'perturbed_validation_set_x' is now obsolte, talk to ****")
            '''
            ...
            if hasattr(self,"perturbed_test_set_x") and self.perturbed_test_set_x is not None:
                ...
                is_perturbed = True
                raise Exception("'perturbed_test_set_x' is now obsolte, talk to ****")
            '''
        ...
        # ****: super cow speed? what do you mean?
        if self._caching_bool:
            dataset = dataset.cache()

        # data augmentation
        if augment:
            # the line below is more elegant, however not compatible with loading PBs (****)
    ...
    def perturb_element(self, *observation):
        ...
        if hasattr(self, "use_mask"):
            perturb_x = tf.concat([tf.expand_dims(mask, 2), perturb_x], axis=2)
    ...
    '''
    # NOT USED ANYMORE SINCE NOT COMPATIBLE WITH PB FILES (****)
    # do not add "_" or this will raise issues when loading the model
    ...
    def get_dataset_with_handle(self, batch_size_train, batch_size_eval):
        ...
        agreed_types = tf.compat.v1.data.get_output_types(train_iterator)
        ...
        # (we want to allow for variable input shapes if the networks permit this, e.g. a fully convolutional) (****)

        agreed_shapes = []

        shapes_ref_list = tf.compat.v1.data.get_output_shapes(datasets[0])
        shapes_equals_list = []
        ...
        iterator = tf.compat.v1.data.Iterator.from_string_handle(handle, agreed_types, agreed_shapes)
