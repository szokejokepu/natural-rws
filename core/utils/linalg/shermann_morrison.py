...
def _shermann_morison_alpha_trick_single_adapt(C, U, compute_uv=True):
    V_T = tf.transpose(U, perm=[1, 0])
    n_z_s = tf.shape(U)[-1]
    k_layer_length = U.shape.as_list()[-2]

    M_inner = tf.matmul(V_T, U)
    lambda_max = tf.linalg.svd(M_inner, compute_uv)[0][0] if compute_uv else tf.linalg.svd(M_inner, compute_uv)[0][0]
    alpha = lambda_max/(C-1)
    M_inner_inv = tf.linalg.inv(alpha * tf.eye(n_z_s, dtype=alpha.dtype) + M_inner)
    M2 = tf.matmul(tf.matmul(U, M_inner_inv), V_T)
