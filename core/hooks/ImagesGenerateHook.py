...
from core.argo.core.hooks.EveryNEpochsTFModelImagesHook import EveryNEpochsTFModelImagesHook
...
class ImagesGenerateHook(EveryNEpochsTFModelImagesHook):
    ...
    def do_when_triggered(self, run_context, run_values):
        ...
        images_sample, images_mean  = self._model.generate(batch_size = self._n_gen_samples, sess=run_context.session)
        ...
        for i in range(rows):
            for j in range(self._n_images_columns):
                panel[i].append(images_mean[c])
                if c == len(images_mean) - 1:
                    break
                else:
                    c = c + 1

        self.images_saver.save_images(panel,
                                 fileName = "generated_probs_" + self._time_ref_shortstr + "_" + str(self._time_ref).zfill(4),
                                 title = self._plot_title,
                                 fontsize=9)
