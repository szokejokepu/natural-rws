...
from ..argo.core.utils.argo_utils import create_reset_metric
...
class PstarImportanceSamplingHook(EveryNEpochsTFModelHook):
    def __init__(self,
                 model,
                 period,
                 time_reference,
        ...
        dirName = dirName + '/pstar_importance_sampling'
        ...
        tf_logging.info("Create ImportanceSampling for pstar %s samples %d repetitions " % (
    ...
    def _begin_once(self):
        ...
        for dataset_str in self._dataset_keys:
            self._mean_values[dataset_str], self._mean_update_ops[dataset_str], self._mean_reset_ops[dataset_str] = zip(
                *[create_reset_metric(tf.compat.v1.metrics.mean,
                                      scope=dataset_str + "_mean_reset_metric/" + tnsr.name + "/" + self._label_n_samples,
                                      values=tnsr) for tnsr in self._tensors_to_average])

    def do_when_triggered(self, run_context, run_values):
        tf_logging.info("trigger for PstarImportanceSampling")
        ...
        for dataset_str in self._dataset_keys:
            ...
            for n in self._n_samples:
                ...
                for i in range(self._repetitions):
                    ...
                    mean_value = np.mean(imp_sampling_group_chunks * 2)
