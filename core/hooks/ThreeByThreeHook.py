...
class ThreeByThreeHook(EveryNEpochsTFModelHook):
    ...
    def _define_true_kl(self):
        ...
        self.probs_gen = tf.math.bincount(tf.map_fn(binary_array_to_int, gen_samples_zero_one), minlength=maximum_bin,
        ...
        self.true_kl = tf.reduce_sum(
            probs_true * (
                tf.math.log(tf.clip_by_value(self.probs_true, 1e-6, 1) / tf.clip_by_value(self.probs_gen, 1e-6, 1))))
        self.uniform_kl2 = tf.reduce_sum(probs_true * (
            tf.math.log(tf.clip_by_value(self.probs_true, 1e-6, 1) / np.asarray([1.0 / maximum_bin] * maximum_bin, dtype=np.float64))))
