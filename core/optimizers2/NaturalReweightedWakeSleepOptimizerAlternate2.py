...
from core.optimizers2.NaturalWakeSleepOptimizerAlternate2 import NaturalWakeSleepOptimizerAlternate2


class NaturalReweightedWakeSleepOptimizerAlternate2(NaturalWakeSleepOptimizerAlternate2):
        ...
        if phase == PHASE_WAKE:
            ...
            for i, layer in enumerate(layers):
                ...
                grad, var = layer.wake(use_natural=(False if i in self._ignore_layers else True), weights=imp_weights_p, global_step=global_step,
                                       layer_name=layer_name, fisher_calculator=self.get_fisher_calculator())
                vars += list(var)
                if len(grad) > 0:
