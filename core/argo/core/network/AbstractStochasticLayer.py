...
from .build_utils import maybe_set_l2_conv_contractive_regularizer, maybe_set_l2_contractive_regularizer_jac
import importlib
...
class AbstractStochasticLayer(AbstractModule):
    ...
    def __init__(self, module_tuple = ("Linear", {}),
                 ...
                 creg_tuple=None, #different from the one above, don't confuse! ("Fisher_Metric", creg_scale)
...
#                 pdb.set_trace()
            ...
            for r in reg_node:
                tf.compat.v1.add_to_collection(ac_collection_name, r)
...
#         import pdb;pdb.set_trace()
        ...
        if self._creg_tuple not in [None, 0.]:
            metric_class, creg_scale = self._creg_tuple
            metric_module = importlib.import_module(".GaussianRegularizers", '.'.join(__name__.split('.')[:-1]))
            metric = getattr(metric_module, metric_class)()
            mean_metric_diag = metric.get_mean_metric(covariance)
            maybe_set_l2_conv_contractive_regularizer(sntmodule_mean, mean, "identity", creg_scale,
                                                      g=mean_metric_diag, name="mean_creg")

            #maybe_set_l2_contractive_regularizer_jac(mean, inputs, creg_scale)
                                                      
            if sntmodule_cov:
                cov_metric_diag = metric.get_cov_metric(covariance)

                #maybe_set_l2_conv_contractive_regularizer(sntmodule_cov, covariance_params, self._cov_parameterization+"_square",
                #                                          creg_scale, g=cov_metric_diag, name="cov_creg")
                #maybe_set_l2_contractive_regularizer_jac(covariance, inputs, creg_scale)
