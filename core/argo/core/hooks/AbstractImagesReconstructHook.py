...
class AbstractImagesReconstructHook(EveryNEpochsTFModelImagesHook):
    ...
    def load_images(self, session):
        ...
        if self._images==None:
            ...
            images = {ds_key : (index_list, self._model.dataset.get_elements(self._model.raw_x, self._ds_handle, self._ds_handles[ds_key], self._ds_initializers[ds_key], session, index_list), self._model.dataset.get_elements(self._model.x_target   , self._ds_handle, self._ds_handles[ds_key], self._ds_initializers[ds_key], session, index_list)) \
