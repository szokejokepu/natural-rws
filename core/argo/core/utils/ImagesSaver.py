...
class ImagesSaver():
    ...
    def save_images(self, panel, fileName, title="", fontsize=9):
        ...
        if panel[0][0].shape[2] == 1:
            images = np.vstack([np.hstack(panel[i]) for i in range(len(panel))])
            Image.fromarray(np.squeeze((np.clip(images,-1,1)+1)/2*255, axis=2).astype(np.uint8)).convert("L").save(self._dirName + '/' + fileName + '_raw.png')
        elif panel[0][0].shape[2] == 3:
            images = np.vstack([np.hstack(panel[i]) for i in range(len(panel))])
            # no squeeze, and other things
            Image.fromarray(((np.clip(images,-1,1)+1)/2*255).astype(np.uint8)).save(self._dirName + '/' + fileName + '_raw.png')
        else:
            raise Exception("Expected number of channels is 1 or 3")
